...
class ZarrEmbeddingCollator:
    ...
    def __call__(self, features):
        ...
        prot_length = batch["prot_input_ids"].shape[1]
        last_hidden_states = np.zeros((batch_size, prot_length, self.embedding_dim), dtype=np.float32)

        for idx, (offset, length) in enumerate(zip(offsets, lengths)):
            l = min(length, prot_length)
